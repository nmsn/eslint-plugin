...
module.exports = {
  meta: {
    type: "suggestion",
    docs: {
      description: "Color strings should not used in jsx attribute",
    ...
    return {
      JSXAttribute(node) {
        ...
        if (type === "Literal" && isColorValue(value)) {
          context.report({
            node,
            message: `Don't use color string '${value}' in jsx attribute`,
          });
          ...
          if (expression.type === "ObjectExpression") {
            ...
            properties.forEach((item) => {
              ...
              if (isColorValue(value.value)) {
                context.report({
                  node: value,
                  message: `Don't use color string '${value.value}' in jsx attribute`,
                });
              }
            });
          }
